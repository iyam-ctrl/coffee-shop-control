...
export default function RecipesPage() {
  ...
  async function loadData() {
    ...
    if (me || !owner) { setError("akun ini belum memiliki akses owner."); setLoading(false); return; }
    setBusinessId(owner.business_id);
    const [p, i, r] = await Promise.all([
      supabase.from("products").select("id, name, sku, selling_price").eq("business_id", owner.business_id).eq("is_active", true).order("name"),
      supabase.from("ingredients").select("id, name, unit_id, current_cost, units(name, symbol)").eq("business_id", owner.business_id).eq("is_active", true).order("name"),
