...
export default async function OwnerDashboard() {
  ...
  return <main style={{ minHeight: "100vh", background: "#090b0d", color: "#f5f1e8", padding: 24 }}><div style={{ maxWidth: 1200, margin: "0 auto" }}>
    <header style={{ display: "flex", justifyContent: "space-between", gap: 20, alignItems: "center", marginBottom: 30, flexWrap: "wrap" }}><div style={{ display: "flex", alignItems: "center", gap: 15 }}><img src="/coffee-shop-control-logo.svg" width="62" height="62" alt="Coffee Shop Control" style={{ borderRadius: 17, boxShadow: "0 10px 35px rgba(197,166,107,.10)" }} /><div><p style={{ color: "#c5a66b", letterSpacing: 2, fontSize: 11 }}>COFFEE SHOP CONTROL</p><h1 style={{ fontSize: 32, margin: "5px 0" }}>owner dashboard</h1><p style={{ color: "#9299a3" }}>{business?.name ?? "bisnis"} · owner</p></div></div><form action="/auth/signout" method="post"><button style={button}>keluar</button></form></header>
    <section style={stats}>{[["omzet", "Rp 0", "hari ini"], ["HPP", "Rp 0", "hari ini"], ["gross profit", "Rp 0", "hari ini"], ["net profit", "Rp 0", "hari ini"], ["nilai stok", "Rp 0", "estimasi saat ini"], ["waste", "Rp 0", "bulan berjalan"]].map(([title, value, note]) => <article key={title} style={card}><p style={muted}>{title}</p><h2 style={{ fontSize: 25, margin: "10px 0 6px" }}>{value}</h2><p style={{ color: "#707782", fontSize: 13 }}>{note}</p></article>)}</section>
    <section style={{ marginTop: 22 }}><div style={{ marginBottom: 13 }}><p style={goldLabel}>CONTROL CENTER</p><h2 style={{ margin: "5px 0" }}>master data & inventory</h2><p style={muted}>alur utama bisnis: produk → recipe → HPP → inventory → transaksi → profit.</p></div><div style={masterGrid}>{masterCards.map(([title, text, href]) => <a key={href} href={href} style={{ textDecoration: "none", color: "inherit" }}><article style={masterCard}><p style={goldLabel}>COFFEE SHOP CONTROL</p><h2 style={{ margin: "8px 0" }}>{title} →</h2><p style={{ color: "#9299a3", lineHeight: 1.6 }}>{text}</p></article></a>)}</div></section>
    <section style={{ marginTop: 22, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: 16 }}>{[["keuangan", "omzet, HPP, laba, biaya, kas & setoran"], ["kontrol", "aktivitas manager, audit log & alert"]].map(([title, text]) => <article key={title} style={card}><h2 style={{ marginBottom: 8 }}>{title}</h2><p style={{ color: "#9299a3", lineHeight: 1.6 }}>{text}</p></article>)}</section>
  </div></main>;
}
