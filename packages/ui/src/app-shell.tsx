      ...
      <aside className={`sidebar ${open ? 'sidebar-open' : ''}`}>
        ...
        <nav className="side-nav" aria-label="navigasi utama">
          {navItems.map((item) => {
            ...
            return <a key={item.href} href={item.href} className={`nav-item ${active ? 'nav-item-active' : ''}`}><span className="nav-dot"/><span><strong>{item.label}</strong>{item.description && <small>{item.description}</small>}</span></a>
          })}
        </nav>
        <div className="sidebar-bottom">
          <a className="account-card" href="/settings"><span className="avatar">A</span><span><strong>Akun & pengaturan</strong><small>profil dan akses</small></span></a>
